...
using NyanSpectreNetlistParser.SpectreNetlistCSTParser: EXPR, Analysis, reducedcontent
using Test
...
function check_roundtrip(var; offset=0)
    s1 = NyanSpectreNetlistParser.parse(var; offset)
    s1_str = reducedcontent(s1)
    s2 = NyanSpectreNetlistParser.parse(s1_str)
    s2_str = reducedcontent(s2)
    @test s1_str == s2_str
end

function check_roundtrip_file(file)
    s1 = NyanSpectreNetlistParser.parsefile(file)
    s1_str = reducedcontent(s1)
    s2 = NyanSpectreNetlistParser.parse(s1_str; fname=file)
    s2_str = reducedcontent(s2)
    @test s1_str == s2_str
end
...
ast = NyanSpectreNetlistParser.parse(analysis)
...
(inparen+1u) (var-(1-x)) ]
"""
ast = NyanSpectreNetlistParser.parse(wave)
