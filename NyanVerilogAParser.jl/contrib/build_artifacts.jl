using Pkg, Pkg.Artifacts, SHA, TreeArchival, NyanVerilogAParser

function lookup_artifact_hash(name)
    artifacts_toml = find_artifacts_toml(Base.pkgdir(NyanVerilogAParser))
...
function build_and_upload_artifact(name, version=Base.pkgversion(NyanVerilogAParser))
    ...
    if local_hash != recorded_hash
        ...
        Pkg.Artifacts.bind_artifact!(
            find_artifacts_toml(Base.pkgdir(NyanVerilogAParser)),
            name,
