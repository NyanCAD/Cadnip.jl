import NyanVerilogAParser
using AbstractTrees
using NyanVerilogAParser.VerilogACSTParser: virtrange
...
macro_va = NyanVerilogAParser.parse("""
    `define f(arg) arg
    `define g(arg) `f(arg)
...
if isfile(bsimcmg_path)
    push!(test_vas, NyanVerilogAParser.parsefile(bsimcmg_path))
end

for va in test_vas
    ls = collect(Leaves(NyanVerilogAParser.VerilogACSTParser.ChunkTree(va.ps)))
