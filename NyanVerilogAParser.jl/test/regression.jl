...
using NyanVerilogAParser.VerilogACSTParser: Node, VerilogSource
using Test
...
let res = NyanVerilogAParser.parse(var)
    ...
    @test rhs.op.op == NyanVerilogAParser.VerilogATokenize.Tokens.PLUS
    # While we're at it, test that strings work.
    @test String(rhs.op) == "+"
end

# Formal argument expansion of first token in a macro (https://github.com/CedarEDA/Cadnip.jl/issues/6)
let va = NyanVerilogAParser.parse("""
...
# Formal argument expansion inside macro arg list (https://github.com/CedarEDA/Cadnip.jl/issues/7)
let va = NyanVerilogAParser.parse("""
       `define f(arg) arg
       `define g(arg) `f(arg)
...
using NyanLexers
@test only(Base.return_types(NyanVerilogAParser.VerilogATokenize.next_token, Tuple{NyanLexers.Lexer{IOBuffer, NyanVerilogAParser.VerilogATokenize.Tokens.Kind,
                                                                          typeof(NyanVerilogAParser.VerilogATokenize.next_token)}, Bool})) ==
    Token{NyanVerilogAParser.VerilogATokenize.Tokens.Kind}
