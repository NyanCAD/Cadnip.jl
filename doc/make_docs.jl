...
# This allows us to push the main `Cadnip` project onto the fron of the stack,
# so that we can load `Cadnip` without having to maintain a separate project here.
...
open(joinpath(@__DIR__, "index.jmd"), write=true) do io
    println(io, "# Cadnip developer documentation index")
    println(io)
