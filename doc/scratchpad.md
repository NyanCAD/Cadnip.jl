...
- [x] Sema/codegen: let a `.model` card read a `.param`. `.model nch nmos vto=vt0` used to fail at load with `UndefVarError` with no override in play, because model cards are hoisted out of the builder as `const`s. Now `model_param_deps` splits the cards: a constant card is still a `const`, a card that reads a `.param` is hoisted as a `@noinline` *function* of exactly those parameters that each scope calls once (top level from its locals, a subcircuit through `parent_params`) — so the large-model SROA hazard the hoisting exists for does not come back. A process corner is now a sweep axis: `Sweep(vt0=…)` moves `vto` on the card. Also deduplicated the two verbatim copies of the model-card lowering onto one `cg_model_value!` — design: `doc/parameter_overrides.md` §4
