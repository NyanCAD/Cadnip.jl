...
- [ ] Bug: sema populates `exposed_models` for MOSFET/OSDI/semiconductor-resistor cards only, so a `.model` card reading a `.param` behind a diode (or BJT, cap, …) in a `.subckt` raises `UndefVarError` — measured on main — design: `doc/parameter_overrides.md` §4
- [ ] Cleanup: delete `.model` card hoisting — measured as dead on the PDK path (35% of the module, 5.7 s of eval, the `const` unread) and ~2% on the deck path; blocked on the `exposed_models` bug above — design/measurements: `doc/parameter_overrides.md` §4
