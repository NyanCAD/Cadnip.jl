...
- [ ] Test brittleness: the `test/mna/vadistiller.jl` anchor-residual check asserts *exact* FP cancellation (`abs(F[i]) < 1e-15` where the row cancels at the ~1190 scale, one ULP ≈ 2.3e-13) — fails on Julia 1.12.7 with 2.8e-14, on `main` as much as on a branch; the leak it guards against was 5e-11 A, so a 1e-12 bound would still catch it
