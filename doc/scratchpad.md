...
- [ ] Two things the README review turned up that are code, not docs:
  - [ ] `src/spc/interface.jl:96` claims "generated code emits fully-qualified references" and needs no `using`. It is stale — codegen still emits `using Cadnip.MNA: Resistor, …` ×4. Dropping them is what would let `sp"..."` work inside a function body, which is what the docs promised for years
