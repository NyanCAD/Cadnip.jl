...
- [x] Sema/codegen: let a `.model` card read a `.param`. `.model nch nmos vto=vt0` used to fail at load with `UndefVarError` with no override in play, because model cards are hoisted out of the builder as `const`s. Now `model_param_deps` splits the cards: a constant card is still a `const`, a card that reads a `.param` is emitted inline in every builder that binds it (top level from its locals, a subcircuit through `parent_params`). Took three designs to get there: a `@noinline` module-level factory of the parameters was built first, to dodge the SROA/per-restamp costs this doc warned of, and measurement on PSP103VA (782 fields) retired all four of those arguments — runtime, allocations, cold compile time and native code size are identical to a constant card, and the factory's generated expression was 2% *larger* than inline, because a card lowers to only the parameters it spells out, never the model's field count. `doc/parameter_overrides.md` §4 records the numbers and the cold-compile measurement trap. A process corner is now a sweep axis: `Sweep(vt0=…)` moves `vto` on the card. Also deduplicated the two verbatim copies of the model-card lowering onto one `cg_model_value!` — design: `doc/parameter_overrides.md` §4
