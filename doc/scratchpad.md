...
- [ ] Codegen: uniquify subcircuit builder names, so two netlists in one module can each define a `.subckt divider`. Unrelated to overrides — found while testing them. **`.model` cards collide the same way and are worse**, because they fail silently rather than by keyword mismatch: two netlists in one module that both write `.model nch nmos level=1 …` share one hoisted card and the *later* one wins, so the earlier netlist's device quietly runs on the other card's parameters (measured: a stage with `lambda=0.01` solved at exactly the λ=0 current because a second, λ-less `nch` was declared below it — no warning, and the only tell was `gds` reading 0). The uniquification wants to cover model cards, not just subcircuit builders
