...
- [x] Diagnose unknown override names — codegen emits a `ParamScope` (declared parameters, child instances, device instances) per *scope* rather than per instance, handed back by an extra `builder(::DeclaredParams)` method so it survives the local scopes generated code lands in; `MNACircuit`/`alter`/sweeps check the override tuple against it and name the fix ("`x1` is an instance, write `x1=(rv=…)`"; device instance parameters are the §1 gap). Two overrides that reached nothing fixed alongside: dotted selectors at construction (`MNACircuit(c; var"x1.r1val"=…)`) and `CircuitSweep`'s base circuit — design: `doc/parameter_overrides.md` §2
