...
- `b_I, b_V`: COO format for RHS vector b (deferred stamps)
...
mutable struct MNAContext
    ...
    # RHS vector b - fully deferred (resolved at assembly time)
    b_I::Vector{MNAIndex}
    b_V::Vector{Float64}
...
All stamps are deferred and resolved at assembly time via `get_rhs()`.
"""
