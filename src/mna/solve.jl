...
"""
struct MNACircuit{U,F,P,S}
    setup::U       # params -> builder (creates OSDI instances if needed)
    builder::F     # cached builder = setup(params)
    params::P
    spec::S
end
...
# Trivial setup wrapper for plain builders (non-OSDI)
_trivial_setup(builder) = (_params) -> builder

# From a builder function (backward compat — wraps in trivial setup)
function MNACircuit(builder, params, spec)
    setup = _trivial_setup(builder)
    MNACircuit(setup, builder, params, spec)
end

# Legacy 4-arg constructor (tspan ignored, for backwards compatibility)
function MNACircuit(builder, params, spec, tspan::Tuple{<:Real,<:Real})
...
Create an MNA circuit from a builder function with keyword parameters.
Parameters are stored as a NamedTuple and can be modified with `alter()`.

For OSDI circuits, pass a setup function instead — see `MNACircuitFromSetup`.
...
```
"""
function MNACircuit(builder; spec=MNASpec(), kwargs...)
    params = NamedTuple(kwargs)
    MNACircuit(builder, params, spec)
end

"""
    MNACircuitFromSetup(setup, params, spec)

Create an MNA circuit from an OSDI setup function.
The setup function has signature `params -> builder` and is re-invoked
by `alter()` when parameters change, creating fresh OSDI instances.
"""
function MNACircuitFromSetup(setup, params, spec)
    builder = setup(params)
    MNACircuit(setup, builder, params, spec)
end

export MNACircuitFromSetup

"""
    alter(circuit::MNACircuit; kwargs...) -> MNACircuit

Create a new circuit with modified parameters.
For OSDI circuits, this re-invokes the setup function to create fresh instances.
...
function alter(circuit::MNACircuit; spec=nothing, kwargs...)
    ...
    # Re-invoke setup to create fresh builder (re-creates OSDI instances if any)
    new_builder = circuit.setup(new_params)
    return MNACircuit(circuit.setup, new_builder, new_params, new_spec)
end
...
function with_spec(circuit::MNACircuit, spec::MNASpec)
    return MNACircuit(circuit.setup, circuit.builder, circuit.params, spec)
end
