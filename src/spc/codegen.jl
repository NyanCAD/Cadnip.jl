    ...
    if va_module_ref !== nothing
        ...
        return quote
            let dev = $va_module_ref(; $(explicit_kwargs...))
                ...
                local _cache_ = $(MNA).make_cache(typeof(dev))
                $(MNA).init_device!(_cache_, dev, spec)
                Base.invokelatest($(MNA).stamp!, dev, ctx, $(port_exprs...), _cache_;
...
function cg_mna_instance!(state::CodegenState, instance::SNode{SP.MOSFET})
    ...
    return quote
        let dev = $device_expr
            ...
            local _cache_ = $(MNA).make_cache(typeof(dev))
            $(MNA).init_device!(_cache_, dev, spec)
            Base.invokelatest($(MNA).stamp!, dev, ctx, $d, $g, $s, $b, _cache_;
                _mna_t_ = t, _mna_mode_ = spec.mode, _mna_x_ = x, _mna_spec_ = spec,
                _mna_instance_ = full_instance_name)
        end
    end
...
function cg_mna_instance!(state::CodegenState, instance::SNode{SP.BipolarTransistor})
    ...
    return quote
        let dev = $device_expr
            ...
            local _cache_ = $(MNA).make_cache(typeof(dev))
            $(MNA).init_device!(_cache_, dev, spec)
            Base.invokelatest($(MNA).stamp!, dev, ctx, $c, $b, $e, $s, _cache_;
                _mna_t_ = t, _mna_mode_ = spec.mode, _mna_x_ = x, _mna_spec_ = spec,
                _mna_instance_ = full_instance_name)
        end
    end
...
function cg_mna_instance!(state::CodegenState, instance::SNode{SP.OSDIDevice})
    ...
    return quote
        let dev = $device_expr
            ...
            local _cache_ = $(MNA).make_cache(typeof(dev))
            $(MNA).init_device!(_cache_, dev, spec)
            Base.invokelatest($(MNA).stamp!, dev, ctx, nodes..., _cache_;
                _mna_t_ = t, _mna_mode_ = spec.mode, _mna_x_ = x, _mna_spec_ = spec,
                _mna_instance_ = full_instance_name)
        end
    end
