...
code = Cadnip.make_spectre_netlist(sa1, [inc])
circuit = eval(code)

@testset "model binning" begin
    @test Cadnip.spicecall(nmos_3p3, l=2.8e-007, w=2.2e-007).device == var"nmos_3p3.0"(L=2.8e-007, W=2.2e-007)
    @test Cadnip.spicecall(nmos_3p3, l=5.0e-007, w=2.2e-007).device == var"nmos_3p3.1"(L=5.0e-007, W=2.2e-007)
end
